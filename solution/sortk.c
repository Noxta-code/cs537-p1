...
int compare_kword(const void *one, const void *two){
    ...
    /*
    // Print the lines and the kth words for debugging
    printf("Comparing:\n%s\n%s\n", line_one[0], line_two[0]);
    if (line_one[1]) printf("Kth word 1: %s\n", line_one[1]);
    if (line_two[1]) printf("Kth word 2: %s\n", line_two[1]);
    printf("%ld\n", strlen(line_one[0]));
    printf("%ld\n", strlen(line_two[0]));
    printf("%ld\n", strlen(line_one[0]) - strlen(line_two[0]));
    */
    ...
    int result = -1;
    if(strlen(line_two[1]) > strlen(line_one[1])){
        result = strncmp(line_one[1], line_two[1], strlen(line_one[1]));
    }
    else{
        result = strncmp(line_one[1], line_two[1], strlen(line_two[1]));
    }
    /*
    printf("First keyword is %s \n", line_one[1]);
    printf("Second keyword is %s \n", line_two[1]);
    printf("Keyword result is %d \n", result);
    */
    ...
    // If kth words were the same, compare whole string
    return strcmp(line_one[0], line_two[0]);

    // If kth words were the same, return the shorter string
    //return strlen(line_one[0]) - strlen(line_two[0]);
}
...
int main(int argc, char *argv[]) {
    ...
    for(int i=0; i<lineNum; i++){
        ...
        // lines[i][0] holds a pointer to the string, so access the last element with strlen(lines[i][i]-1)
